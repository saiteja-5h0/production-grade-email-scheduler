...
export async function notifyRateLimit(senderEmail: string, retryAt: number) {
  const key = `rate-limit-notified:${senderEmail}:${new Date().toISOString().slice(0, 13)}`;
  if (!(await redis.set(key, "1", "EX", 7200, "NX"))) return;
  const connection = await prisma.slackConnection.findUnique({ where: { userId: "demo-user" } });
